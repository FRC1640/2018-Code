	...
	public Start2Sc2Cube1(IRobotContext robotContext, FieldConfig config) {
		...
		if (dir != 0) {
			...
			if (dir == scSide) { // on same side as scale
				addCommand("start-to-scale", new Start2Sc(robotContext, config));
				addCommand("wait-start-to-scale", new WaitCommand(getCommand("start-to-scale")));
				addCommand("drive-to-alley", new DriveStraight(robotContext, 98, 180, 0.7));
				...
				addCommand("cross-the-line", new CrossTheLine(robotContext, config));
			}
		}
