...
public class SwitchSwitchCross extends AutonScript {

	public SwitchSwitchCross(IRobotContext robotContext, FieldConfig config) {
		...
		ShortestAngleSunflower ocelotSunflowerTurn = new ShortestAngleSunflower(driveTrain, gyro, fieldCentric, 0.0105, 0.00003, 0.0005, 0.9);
		...
		default: System.out.println("SwitchSwitchCross: Invalid starting location: " + config.getStartPos().name());
		}
		...
		default: System.out.println("SwitchSwitchCross: Invalid scale position: " + config.getScPos().name());
		}
		switch(config.getSwPos()) {
		case Left: swSide = -1; break;
		case Right: swSide = 1; break;
		default: System.out.println("SwitchSwitchCross: Invalid switch position: " + config.getSwPos().name());
		}
...
//				addCommand("drive-alley", new DriveStraight(robotContext, 40, dir*90, 1.0)); 
