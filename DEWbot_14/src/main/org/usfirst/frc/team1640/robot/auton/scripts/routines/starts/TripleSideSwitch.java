...
public class TripleSideSwitch extends AutonScript {

	public TripleSideSwitch(IRobotContext robotContext, FieldConfig config) {
		...
		ShortestAngleSunflower ocelotSunflowerTurn = new ShortestAngleSunflower(driveTrain, gyro, fieldCentric, 0.02, 0.000031, 0.0005, 0.9);
...
//				addCommand("drive-back-2", new DriveUntilInches(driveTrain, sunflower, 9, -dir*25, 0.6)); 
				addCommand("placer-to-transport", new PlacerPresetCommand(robotContext, PlacerPreset.CubeTravel));
				addCommand("time-turn-2", new TimeCommand(0.3));
				addCommand("wait-time-turn-1", new WaitCommand(getCommand("time-turn-2")));
//				addCommand("turn-to-alley-1", new TurnAndMaintainDirection(driveTrain, ocelotSunflowerTurn, gyro, 180, 6, 0.1, 20));
...
//				addCommand("wait-turn-to-alley-1", new WaitCommand(getCommand("turn-to-alley-1")));
				...
				addCommand("approach-scale-1", new DriveUntilSeconds(ocelotSunflower, 1.1, dir*-160, 0.6)); 
				addCommand("wait-time-turn-1", new WaitCommand(getCommand("approach-scale-1")));
				
				addCommand("start-intake", new SetIntakeDrive(intakeStrat2, -1));


//				addCommand("time-turn-1", new TimeCommand(0.05));
				
//				addCommand("placer-to-transport", new PlacerPresetCommand(robotContext, PlacerPreset.CubeTravel));
//				addCommand("drive-alley", new DriveUntilInches(driveTrain, ocelotSunflower, 40, dir*90, 1.0)); 
